    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
        <DialogHeader className="pr-8">
          ...
          <DialogDescription>
            Revisa las {group.candidates.length} propuestas antes de compartir
            el paquete. Puedes abrir cualquiera para corregir sus datos.
          </DialogDescription>
        </DialogHeader>

        <nav
          className="flex gap-2 overflow-x-auto border-y py-3"
          aria-label="Propuestas del paquete"
        >
        ...
        <article className="space-y-4 rounded-2xl border bg-background p-4 sm:p-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline">
              Duplicidad {active.duplicate.score}%
            </Badge>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <PreviewField label="Servicio de Adecco" value={active.service} />
            ...
            <Button
              ...
              onClick={() =>
                setActiveIndex((value) =>
                  Math.min(group.candidates.length - 1, value + 1),
                )
              }
              disabled={activeIndex === group.candidates.length - 1}
            >
