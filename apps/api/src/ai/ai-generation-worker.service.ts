import {
  Injectable,
  Logger,
  OnModuleDestroy,
  OnModuleInit,
} from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { Worker } from 'bullmq';
import { WorkerSupervisor } from '../common/queues/worker-supervisor';
import {
  BULLMQ_PREFIX,
  redisConnection,
} from '../titles/title-evaluation-queue.service';
import {
  AI_GENERATION_QUEUE,
  type AiGenerationJobData,
} from './ai-generation-queue.service';
import { AiGenerationProcessorService } from './ai-generation-processor.service';

@Injectable()
export class AiGenerationWorkerService
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(AiGenerationWorkerService.name);
  private worker?: Worker<AiGenerationJobData>;
  private supervisor?: WorkerSupervisor;

  constructor(
    private readonly config: ConfigService,
    private readonly processor: AiGenerationProcessorService,
  ) {}

  onModuleInit(): void {
    if (
      !this.config.get<boolean>('AI_GENERATION_ENABLED') ||
      !this.config.get<boolean>('AI_GENERATION_WORKER_ENABLED')
    ) {
      return;
    }
    const timeoutMs = this.config.getOrThrow<number>(
      'AI_GENERATION_TIMEOUT_MS',
    );
    this.worker = new Worker<AiGenerationJobData>(
      AI_GENERATION_QUEUE,
      async (job) => {
        try {
          return await this.processor.process(
            job.data.runId,
            Date.now() + timeoutMs,
          );
        } catch (value) {
          const error =
            value instanceof Error ? value : new Error(String(value));
          const attempt = job.attemptsMade + 1;
          const finalAttempt = attempt >= (job.opts.attempts ?? 1);
          await this.processor.recordFailure(
            job.data.runId,
            error,
            finalAttempt,
            attempt,
          );
          throw error;
        }
      },
      {
        prefix: BULLMQ_PREFIX,
        connection: redisConnection(
          this.config.getOrThrow<string>('REDIS_URL'),
          true,
        ),
        concurrency: this.config.getOrThrow<number>(
          'AI_GENERATION_CONCURRENCY',
        ),
        lockDuration: timeoutMs + 30_000,
        maxStalledCount: 1,
        autorun: false,
      },
    );
    this.worker.on('completed', (job) =>
      this.logger.log(`Automatización ${job.id ?? 'sin-id'} completada.`),
    );
    this.worker.on('failed', (job, error) =>
      this.logger.warn(
        `Automatización ${job?.id ?? 'sin-id'} falló: ${error.message}`,
      ),
    );
    this.worker.on('error', (error) =>
      this.logger.error(`Error del trabajador editorial: ${error.message}`),
    );
    this.supervisor = new WorkerSupervisor(
      this.worker,
      this.logger,
      'El trabajador editorial',
    );
    this.supervisor.start();
  }

  async onModuleDestroy(): Promise<void> {
    await this.supervisor?.close();
  }
}
